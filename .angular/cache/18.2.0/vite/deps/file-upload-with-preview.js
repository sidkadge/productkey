...
var b = class {
  constructor(A, g = {}) {
    ...
    this.options.showDeleteButtonOnImages = i ?? true, this.options.maxFileCount = C ?? 0, this.options.presetFiles = I ?? [], this.options.multiple = E ?? false, this.options.accept = B ?? this.options.accept;
    const {
      browse: t,
      chooseFile: d,
      label: u,
    ...
    if (!s) throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`);
    ...
    if (o != null && a != null && n != null && r != null) this.inputHidden = o, this.inputVisible = a, this.inputVisible.innerHTML = this.options.text.chooseFile, this.imagePreview = n, this.clearButton = r;
    else throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`);
    const {
      backgroundImage: S,
      baseImage: h,
      successFileAltImage: J,
      successPdfImage: R,
      successVideoImage: p
    } = g.images || {};
  ...
  bindClickEvents() {
    ...
    }, true), this.clearButton.addEventListener("click", () => {
      const A = {
        detail: {
    ...
    }, true), this.imagePreview.addEventListener("click", (A) => {
      const g = A.target;
      if (g) {
        if (g.matches(".custom-file-container .image-preview-item-clear-icon")) {
          const B = g.getAttribute("data-upload-name"), C = this.cachedFileArray.findIndex(({
            name: E
          }) => E === B);
          this.deleteFileAtIndex(C);
        }
        if (g.matches(".custom-file-container .image-preview-item")) {
          const B = g.querySelector(".image-preview-item-clear-icon"), C = B == null ? void 0 : B.getAttribute("data-upload-name"), E = this.cachedFileArray.findIndex(({
            name: t
          }) => t === C);
          if (E < 0) return;
          const I = {
  ...
  addImagesFromPath(A) {
    return __async(this, null, function* () {
      A.forEach((g) => __async(this, null, function* () {
        try {
          const B = "image/jpeg", E = yield (yield fetch(g, {
            mode: "cors"
          })).blob(), I = new File([E], "preset-file", {
            type: E.type || B
          });
          this.addFiles([I]);
        } catch (B) {
  ...
  addFiles(A) {
    ...
    this.options.multiple || (this.cachedFileArray = []), g.forEach((E) => {
      const I = new File([E], `${E.name || "fallback-name"}${l}${U()}`, {
        type: E.type
      });
  ...
  deleteFileAtIndex(A) {
    ...
    this.cachedFileArray = [...this.cachedFileArray.slice(0, A), ...this.cachedFileArray.slice(A + 1)], this.refreshPreviewPanel();
    const g = {
      detail: {
        cachedFileArray: this.cachedFileArray,
...
//# sourceMappingURL=file-upload-with-preview.js.map
