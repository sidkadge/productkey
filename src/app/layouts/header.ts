...
export class HeaderComponent {
  ...
  ngOnInit() {
    this.checkUserSession(); // Check session on component initialization
  ...
  checkUserSession() {
    // Check if the necessary data exists in localStorage (e.g., sessionId or token)
    const sessionId = localStorage.getItem("sessionId");
    const tokenExpiration = localStorage.getItem("tokenExpiration");

    if (!sessionId || !tokenExpiration) {
      // Redirect to sign-up page if data is missing
      this.router.navigate(["/auth/signin"]);
    } else {
      // Proceed with your app logic if session data is present
      this.loadUserData(); // (if you have additional logic for loading user data)
    }
  }

  loadUserData() {
    // Example of loading user data if session exists
    this.user = {
      /* Load user data here, for example, from an API or store */
    };
  }
  ...
  logout() {
    // Clear all data in localStorage and sessionStorage
    ...
    // Optionally navigate to a login or other page after logout
  }
}
